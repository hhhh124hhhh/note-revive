...
; 基本设置
Name "${APPNAME}"
OutFile "${APPNAME}-Setup-Chinese-Final-${VERSION}.exe"
...
!define MUI_WELCOMEPAGE_TEXT "欢迎使用 ${APPNAME} - ${DESCRIPTION}！$\r$\n$\r$\n本应用将帮助您高效管理各种笔记和想法。$\r$\n$\r$\n版本：${VERSION}$\r$\n开发者：${PUBLISHER}$\r$\n联系邮箱：${EMAIL}$\r$\n$\r$\n点击「下一步」继续安装。"

!define MUI_FINISHPAGE_TITLE "安装完成"
!define MUI_FINISHPAGE_TEXT "${APPNAME} 已成功安装到您的计算机上。$\r$\n$\r$\n您现在可以开始使用这款智能便签管理应用了。$\r$\n$\r$\n如果遇到任何问题，请联系开发者：${EMAIL}"

!define MUI_DIRECTORYPAGE_TEXT_TOP "选择安装目录"
!define MUI_DIRECTORYPAGE_TEXT_DESTINATION "请选择 ${APPNAME} 的安装位置："

!define MUI_INSTFILESPAGE_TEXT_TOP "正在安装"
!define MUI_INSTFILESPAGE_FINISH_TEXT_TEXT "完成"
