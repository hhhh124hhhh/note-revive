...
; 基本设置
Name "${APPNAME}"
OutFile "${APPNAME}-Setup-Fixed-${VERSION}.exe"
...
Section "${APPNAME}" SecMain
    ...
    CreateShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\Note Revive.exe" "" "$INSTDIR\icons\icon.ico" 0 SW_SHOWNORMAL "" "${CHINESE_DESC}"
    CreateShortCut "$SMPROGRAMS\${APPNAME}\卸载 ${APPNAME}.lnk" "$INSTDIR\Uninstall.exe"
    
    ; 创建桌面快捷方式（可选，通过完成页面创建）
    ; CreateShortCut "$DESKTOP\${APPNAME}.lnk" "$INSTDIR\Note Revive.exe" "" "$INSTDIR\icons\icon.ico" 0 SW_SHOWNORMAL "" "${CHINESE_DESC}"
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${APPNAME}" "DisplayIcon" "$INSTDIR\icons\icon.ico"
...
Function CreateDesktopShortcut
    ...
    ; 显示创建成功消息
    MessageBox MB_OK "桌面快捷方式已创建！$\r$\n$\r$\n您现在可以在桌面上看到 ${APPNAME} 图标了。"
FunctionEnd
