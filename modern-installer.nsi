...
; 基本设置
Name "${APPNAME}"
OutFile "${APPNAME}-Setup-Modern-${VERSION}.exe"
...
; 现代UI样式设置
!define MUI_UI "F:/person/3-数字化集锦/note-revive/.nsis/nsis-3.08/Contrib/UIs/modern.exe"
!define MUI_UI_HEADERIMAGE "F:/person/3-数字化集锦/note-revive/.nsis/nsis-3.08/Contrib/UIs/modern_headerbmp.exe"
!define MUI_UI_HEADERIMAGE_RIGHT "F:/person/3-数字化集锦/note-revive/.nsis/nsis-3.08/Contrib/UIs/modern_headerbmpr.exe"
...
!define MUI_INSTFILESPAGE_ABORT_TEXT "用户取消了安装"

!define MUI_UNWELCOMEPAGE_TITLE "卸载 ${APPNAME}"
!define MUI_UNWELCOMEPAGE_TEXT "此向导将引导您卸载 ${APPNAME}。$\r$\n$\r$\n您的个人数据将保留在用户目录中。"

!define MUI_UNFINISHPAGE_TITLE "卸载完成"
!define MUI_UNFINISHPAGE_TEXT "${APPNAME} 已从您的计算机中移除。$\r$\n$\r$\n感谢您的使用！"
...
Section "${APPNAME}" SecMain
    ...
    ; 显示安装信息
    DetailPrint "正在安装 ${APPNAME} 主程序..."
    
    ; 复制主程序文件
    File "dist-portable\Note Revive.exe"
    DetailPrint "复制主程序文件完成"
    
    File "dist-portable\README.md"
    DetailPrint "复制文档文件完成"
    
    ; 创建数据目录
    CreateDirectory "$APPDATA\${APPNAME}"
    DetailPrint "创建用户数据目录：$APPDATA\${APPNAME}"
    ...
    WriteRegStr HKCU "Software\${APPNAME}" "Publisher" "${PUBLISHER}"
    WriteRegStr HKCU "Software\${APPNAME}" "Email" "${EMAIL}"
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${APPNAME}" "InstallDate" "$%DATE% $%TIME%"
